...
// Coordinates are plotted directly in the outline's own viewBox space, estimated from each
// city's real latitude/longitude mapped proportionally onto India's bounding box — close
// enough for an illustrative network map, not a surveyed/GIS-accurate one.
const hub: CityPin = { name: "Mumbai", x: 112, y: 480 };

const cities: CityPin[] = [
  { name: "Delhi NCR", x: 212, y: 225 },
  { name: "Ahmedabad", x: 105, y: 375 },
  { name: "Pune", x: 135, y: 495 },
  { name: "Nagpur", x: 255, y: 425 },
  { name: "Hyderabad", x: 241, y: 525 },
  { name: "Bengaluru", x: 221, y: 644 },
];

// Simplified single-path outline of India (no state borders), traced from the public-domain
// "India outline.svg" reference on Wikimedia Commons — a flat silhouette, not a political map.
...
/** Bézier control point offset perpendicular to the hub→city line, so each connector reads
 *  as a gentle arc rather than a straight ruler line. */
function arcControlPoint(x1: number, y1: number, x2: number, y2: number) {
  const mx = (x1 + x2) / 2;
  const my = (y1 + y2) / 2;
  const dx = x2 - x1;
  const dy = y2 - y1;
...
/** A real, simplified India outline (no state borders) with pins plotted at each partner
 *  city's approximate real position and curved connectors fanning out from the Mumbai HQ —
 *  replaces the earlier abstract "stylised footprint" panel with an actual map. */
          ...
          <AnimatedSection animation="scale">
            <div className="mx-auto w-full max-w-2xl rounded-[32px] bg-[#FBFAF6] p-4 sm:p-5 lg:p-5">
              <svg viewBox="0 0 666.66669 777.33331" className="mx-auto h-auto w-full" role="img" aria-label="Map of India showing Tech4Bharat's partner cities">
                ...
                {cities.map((city) => {
                  const c = arcControlPoint(hub.x, hub.y, city.x, city.y);
                  return (
                    <path
                      key={city.name}
                      d={`M ${hub.x} ${hub.y} Q ${c.x} ${c.y} ${city.x} ${city.y}`}
                      fill="none"
                      stroke="#1F4E3D"
                      strokeWidth="1.5"
                      strokeOpacity="0.3"
