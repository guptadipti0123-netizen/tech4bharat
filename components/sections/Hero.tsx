...
/** Home hero — a real event photo (a Tech4Bharat hackathon session) as a true full-bleed
 *  background, using object-cover so it fills the entire hero at every breakpoint with no
 *  letterboxing — the earlier object-contain treatment left visible bands of plain gradient
 *  above/below the photo on mobile, which read as "half image, half flat color" and looked
 *  unfinished, so this switches to a full-cover photo (the fallback gradient behind it is
 *  now only a load-in placeholder, never visible once the image paints) with a dark wash on
 *  top so the white/gold heading and buttons stay readable. */
export default function Hero() {
  return (
    ...
    >
      {/* Event photo — object-cover fills the entire hero background at every breakpoint,
          so there is no visible letterboxing/gradient band around it. */}
      <div className="absolute inset-0">
        <Image
          src="/images/gallery/hackathons-1.jpg"
          alt="Founders collaborating at a Tech4Bharat hackathon session"
          fill
          priority
          sizes="100vw"
